...
## 2024-11-26 - Escaping Data in Inline Event Handlers\n**Learning:** When interpolating dynamic data (like Eloquent model attributes) into inline JavaScript event handlers within Blade templates (e.g., `onsubmit="return confirm(\x27Delete {{ $model->title }}\x27)"`), unescaped quotes in the data will cause JavaScript syntax errors and potentially introduce XSS vulnerabilities.\n**Action:** Always escape dynamic data interpolated into inline JavaScript event handlers using `addslashes()` or equivalent (e.g., `{{ addslashes($model->title) }}`).
