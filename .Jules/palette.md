...
## "2026-06-26" - Integrating Mock UI Elements
**Learning:** When using HTML templates, mock action elements (like styled anchor tags for editing/deleting) must be fully integrated with backend functionality and updated with specific ARIA labels instead of leaving duplicate generic functional links next to them, as this creates confusing duplicate paths for screen readers.
**Action:** Always verify that beautifully styled template buttons are the ones wired to the actual backend endpoints and have dynamic, specific aria-labels based on the looped model.
