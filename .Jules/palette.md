...
## 2024-11-20 - Unify Template UI Links with Real Backend Routes
**Learning:** This admin panel often contains styled HTML template "mock" links with empty or `#!` hrefs located directly adjacent to unstyled, functional text links generated by Laravel backend logic.
**Action:** When identifying UX enhancements, specifically look for opportunities to merge functional backend routes (especially those requiring `<form>` submissions like DELETE actions) into the beautifully styled template icons, ensuring descriptive `aria-label` attributes are added to the previously generic template buttons.
