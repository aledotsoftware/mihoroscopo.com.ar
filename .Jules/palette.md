...
## 2024-11-25 - Provide context in generic actions\n**Learning:** Buttons inside lists or tables often contain generic text such as 'Delete', 'Edit', or 'Choose Plan'. This lacks context for screen reader users when navigating interactively out of context.\n**Action:** Add descriptive 'aria-label' attributes that include the subject's name, e.g., aria-label="Elegir plan Diario".
## 2024-11-25 - Safety Rule: Empty Action\n**Learning:** The form in resources/views/admin/articles/index.blade.php uses an empty action attribute (action=""). Submitting it to the index route with a DELETE method produces a MethodNotAllowedHttpException.\n**Action:** Form actions must point to the specific route, e.g. action="/admin/articles/{{ $article->id }}".
