...
## 2026-05-21 - Accessible and Safe Action Buttons in Repeating Tables
**Learning:** Replaced cluttered text links and non-functional styled icon buttons with a unified set of accessible, functional SVG icon buttons. In repeating data tables, action buttons with generic icons (edit/delete) must include context-specific `aria-label` attributes (e.g., `aria-label="Eliminar artículo: {{ $article->title }}"`) for screen readers. Furthermore, when adding inline JavaScript confirmation dialogs, interpolating Blade variables like `$article->title` can break the JS syntax or introduce XSS vulnerabilities if the string contains unescaped quotes.
**Action:** Always merge mock styled buttons with functional logic when integrating HTML admin templates. Ensure decorative SVGs have `aria-hidden="true"`, action buttons have descriptive `aria-label`s with the item's name/title, and dynamically injected JS strings are always safely escaped using `addslashes()`.
