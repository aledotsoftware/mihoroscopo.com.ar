...
## 2026-06-24 - Integrated Admin Template Actions
**Learning:** When integrating downloaded HTML admin templates, the action columns often contain beautifully styled mock buttons alongside plain text placeholder functionality. It's crucial to merge the two properly, mapping real forms and route IDs to the styled elements, adding context-aware `aria-label`s for screen readers (since template icons are usually unlabelled), and applying `aria-hidden="true"` to the SVGs.
**Action:** Always verify if a data table's action buttons are actually functional or just mockups. When hooking up real forms to styled template buttons, ensure you carry over or add necessary `aria-label` attributes and explicitly hide decorative icons.
