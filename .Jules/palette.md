...
## 2024-05-26 - Merge mock UI with functional forms and dynamic context
**Learning:** Admin template index views often contain beautifully styled mock buttons with icons (e.g., `<a href="#!">...</a>`). Leaving these visually disconnected from the actual backend functional links/forms creates confusing plain-text redundant links. Additionally, applying generic `aria-label="Eliminar artículo"` on repeating table rows fails to provide screen readers with context of *which* item is being deleted.
**Action:** Always replace the `href="#!"` in the mock UI elements with the functional backend route or wrap them in the proper form. Use dynamic item-specific interpolation (e.g., `aria-label="Eliminar artículo: {{ $article->title }}"` and `confirm('... {{ addslashes($article->title) }}?')`) to give full context to screen readers and sighted users during destructive actions. Add `aria-hidden="true"` to purely decorative SVGs within these interactive elements.
