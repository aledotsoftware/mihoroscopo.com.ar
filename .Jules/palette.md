...
## 2026-06-21 - Error Handling UX Improvement\n**Learning:** When async API calls fail, silently reloading the page is a destructive pattern that forces users to lose all form context without explanation. Existing validation or error modals in the DOM should be reused for API catch blocks instead of hard reloads.\n**Action:** Always intercept  blocks in async flows to provide informative error state using existing UI components (like ) and explicitly return focus to the triggering element.
## 2026-06-21 - Error Handling UX Improvement\n**Learning:** When async API calls fail, silently reloading the page is a destructive pattern that forces users to lose all form context without explanation. Existing validation or error modals in the DOM should be reused for API catch blocks instead of hard reloads.\n**Action:** Always intercept `.catch()` blocks in async flows to provide informative error state using existing UI components (like `showModal()`) and explicitly return focus to the triggering element.
