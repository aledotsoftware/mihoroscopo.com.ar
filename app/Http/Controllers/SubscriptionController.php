...
class SubscriptionController extends Controller
{
    ...
    public function subscribe(Request $request)
    {
        ...
        // What: Added an explicit list of columns to fetch when resolving an existing subscription.
        // Why: The 'subscriptions' table contains massive TEXT/JSON columns ('response', 'payload').
        //      Hydrating these into the Eloquent model during the highly concurrent subscribe checkout flow wastes significant RAM and CPU,
        //      since we only need to verify the status or update basic references.
        // Impact: Reduces memory footprint and database query time per checkout.
        $existingSubscription = $this->getSubscriptionByEmail($email, ['id', 'status', 'external_reference', 'email', 'payment_provider_id']);
    ...
    public function reactivateSubscription($externalReference, $paymentType)
    {
        ...
        // ⚡ Bolt: Memory & CPU optimization.
        // What: Added an explicit list of columns to fetch when resolving a subscription for reactivation.
        // Why: Similar to checkout, fetching the full record with large TEXT/JSON fields is unnecessary.
        //      We only need fields necessary to call MercadoPago and perform the update.
        // Impact: Eliminates unneeded memory allocation and network payload size.
