...
class SubscriptionController extends Controller
{
     ...
     * @param array $columns Columnas a seleccionar para optimizar memoria en tablas pesadas.
    ...
    public function subscribe(Request $request)
    {
        ...
        // What: Restricted columns fetched using select() in the getSubscriptionByEmail method.
        // Why: The 'subscriptions' table contains large JSON columns ('response', 'payload').
        //      Hydrating these into memory during the highly concurrent checkout flow wastes resources
        //      when we only need status, external_reference, and payment_provider_id to check existing subscriptions.
        // Impact: Eliminates significant memory allocation and CPU overhead per checkout request.
    ...
    public function reactivateSubscription($externalReference, $paymentType)
    {
        ...
        // ⚡ Bolt: Memory & CPU optimization.
        // What: Restricted columns fetched using select() in the getSubscriptionByExternalReference method.
        // Why: The 'subscriptions' table contains large JSON columns ('response', 'payload').
        //      Hydrating these into memory wastes resources when we only need specific fields to check status
        //      and perform reactivation (email, service_id, payment_type, subscription_id, status).
        // Impact: Eliminates significant memory allocation and CPU overhead per reactivation request.
