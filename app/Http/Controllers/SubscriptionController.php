...
class SubscriptionController extends Controller
{
    ...
    private function getSubscriptionByEmail($email, $columns = ['*'])
    {
        ...
        // What: Added optional $columns parameter and chained select($columns) to the query.
        // Why: The 'subscriptions' table contains large TEXT/JSON columns ('response', 'payload').
        //      In high-concurrency flows (like checkout), hydrating these columns wastes significant memory and CPU.
        //      Adding a $columns array allows callers to specify only needed fields without breaking other usages.
        // Impact: Reduces memory footprint and query time during the critical path of checkout creation.
    ...
    private function getSubscriptionByExternalReference($externalReference, $columns = ['*'])
    {
        // ⚡ Bolt: Memory optimization.
        // What: Added optional $columns parameter and chained select($columns) to the query.
        // Why: Prevents hydrating the full model with heavy JSON columns when resolving a single subscription.
        // Impact: Eliminates redundant database transfer and memory overhead per request.
    ...
    public function subscribe(Request $request)
    {
        ...
        $existingSubscription = $this->getSubscriptionByEmail($email, ['id', 'status', 'external_reference', 'payment_provider_id', 'email']);
