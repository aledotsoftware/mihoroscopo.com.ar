...
class SubscriptionController extends Controller
{
    ...
    public function subscribe(Request $request)
    {
        ...
        // What: Added select() to restrict columns loaded when checking for an existing subscription.
        // Why: The 'subscriptions' table contains large JSON/TEXT columns ('response', 'payload') that are
        //      extremely heavy to hydrate into memory. The subscribe endpoint only needs basic fields like status and references.
        // Impact: Reduces memory usage and CPU hydration overhead significantly on a high-throughput endpoint.
    ...
    public function reactivateSubscription($externalReference, $paymentType)
    {
        ...
        // ⚡ Bolt: Memory optimization.
        // What: Added select() to limit the columns hydrated during the reactivation flow.
        // Why: The reactivation flow only needs core identifier and status fields. Loading massive TEXT columns
        //      wastes RAM and CPU without providing any value for the MercadoPago api call and update.
        // Impact: Lowers memory overhead when users reactivate their subscriptions.
