...
class SubscriptionController extends Controller
{
     ...
     * @param  array  $columns  Las columnas a retornar para optimizar la memoria.
    ...
    private function getSubscriptionByEmail($email, $columns = ['*'])
    {
        ...
        // Added $columns parameter to avoid hydrating massive JSON columns ('response', 'payload')
        // into memory for single record lookups in high-concurrency flows like 'subscribe'.
        return Subscription::where('email', $email)->select($columns)->first();
    }

    /**
     * Obtiene una suscripción existente por reference externa.
     *
    ...
    private function getSubscriptionByExternalReference($externalReference, $columns = ['*'])
    {
        // ⚡ Bolt: Memory & CPU optimization.
        // Added $columns parameter to avoid hydrating massive JSON columns ('response', 'payload')
        // into memory for single record lookups in high-concurrency flows like 'reactivateSubscription'.
        return Subscription::where('external_reference', $externalReference)->select($columns)->first();
    }
    ...
    public function subscribe(Request $request)
    {
        ...
        $existingSubscription = $this->getSubscriptionByEmail($email, ['id', 'status', 'external_reference', 'email']);
    ...
    public function reactivateSubscription($externalReference, $paymentType)
    {
        ...
        $subscription = $this->getSubscriptionByExternalReference($externalReference, ['id', 'email', 'payment_type', 'subscription_id', 'status', 'service_id']);
    ...
    // What: Replaced large switch statement with a static array map.
    // Why: A string-based switch statement processes comparisons sequentially, taking O(N) worst-case time for unmatched or late-matching values. A static array map offers O(1) constant-time lookup, which saves CPU cycles and is much cleaner, especially in high-throughput endpoints.
    // Impact: Minor CPU savings per call, but provides cleaner code and better scalability for frequent mappings.
