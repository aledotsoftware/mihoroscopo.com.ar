...
## 2024-05-30 - [Select Column Omission Breaking Checkout Logic]
**Learning:** When using `select()` to optimize memory usage by omitting massive TEXT/JSON columns (like `payload`), it's extremely easy to accidentally omit columns that are implicitly required by conditional logic further down the flow. In `SubscriptionController@reactivateSubscription`, restricting the select to `['id', 'email']` broke the checkout flow because the subsequent logic required `service_id`, `payment_type`, `subscription_id`, and `status` to be saved back to the database. Omitting them caused Eloquent to potentially overwrite existing data with nulls or fail to update properly.
**Action:** Always meticulously audit every property accessed on an Eloquent model within the entire method scope (including conditionals and saving) before applying a restricted `select()` array. Default to selecting more columns if unsure, prioritizing application correctness over micro-optimizing a few bytes.
