...
## 2026-05-18 - [Avoid SELECT * in generic model getters with heavy columns]
**Learning:** Generic getter methods like `getSubscriptionByEmail()` fetching `Subscription::where(...)->first()` trigger full model hydration, including massive TEXT/JSON payload columns like `response` and `payload`. This causes massive memory spikes during high-concurrency flows like checkout or webhooks.
**Action:** Explicitly use `select()` in these getters to fetch only the columns required by their callers, avoiding unnecessary data transfer and hydration of heavy columns.
