...
\n## 2026-05-18 - [Eloquent Model Hydration Overhead in Single Record Lookups]\n**Learning:** In high-concurrency flows like subscription checkouts and reactivations, generic getter methods like `getSubscriptionByEmail` that return a full Eloquent model without restricting columns unnecessarily hydrate massive TEXT/JSON columns (like `response` or `payload`) into memory. This causes massive memory overhead and wastes CPU cycles.\n**Action:** When using generic lookup methods in high-throughput endpoints, modify the getter signature to accept an optional `$columns = ['*']` parameter and pass it to `select()`. Then update the callers in performance-critical areas to explicitly fetch only the specific columns they require to drastically reduce memory usage.
