...
## 2024-05-21 - [Select Specific Columns on High-Concurrency Lookups]
**Learning:** During highly concurrent flows like the `subscribe` and `reactivateSubscription` endpoints, looking up a single record (e.g., using `getSubscriptionByEmail` or `getSubscriptionByExternalReference`) with `->first()` fetches all columns by default. The `subscriptions` table contains a massive `response` TEXT/JSON column, which consumes significant memory and CPU to hydrate into the Eloquent model, even if the view/controller only uses a few string fields like `status` or `external_reference`.
**Action:** When creating generic "getter" methods for Eloquent models (e.g., `getSubscriptionByEmail`), always add an optional array parameter for `$columns = ['*']` and pass it to a `select()` clause before `->first()`. In highly concurrent paths that invoke these methods, pass an explicit array of only the necessary columns (e.g., `['id', 'email', 'status', 'external_reference']`) to prevent hydrating heavy payload columns.
