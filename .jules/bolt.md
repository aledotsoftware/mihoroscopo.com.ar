...
## 2026-06-08 - [Avoid Select * on Highly Concurrent Controller Actions]
**Learning:** The  method is a high-concurrency checkout flow that calls `getSubscriptionByEmail()` which previously performed a `SELECT *` on the `subscriptions` table. The `subscriptions` table contains massive JSON columns like `payload` and `response` which waste large amounts of RAM and CPU when hydrated into an Eloquent Model simply to check a status.
**Action:** Modify `getSubscriptionByEmail()` to accept an optional `$columns` array parameter, and pass the explicit columns needed (`id`, `status`, `external_reference`) from `subscribe()` to drastically reduce database payload sizes and memory consumption during checkout peaks.
## 2026-05-19 - [Avoid Select * on Highly Concurrent Controller Actions]
**Learning:** The `subscribe` method is a high-concurrency checkout flow that calls `getSubscriptionByEmail()` which previously performed a `SELECT *` on the `subscriptions` table. The `subscriptions` table contains massive JSON columns like `payload` and `response` which waste large amounts of RAM and CPU when hydrated into an Eloquent Model simply to check a status.
**Action:** Modify `getSubscriptionByEmail()` to accept an optional `$columns` array parameter, and pass the explicit columns needed (`id`, `status`, `external_reference`) from `subscribe()` to drastically reduce database payload sizes and memory consumption during checkout peaks.
