...
## 2026-05-24 - [Avoid Type/Pollution Errors in Cache Keys from Request Parameters]
**Learning:** Using raw `request()->get('page')` to fetch query parameters for constructing cache keys (e.g., `Cache::remember('articles_index_page_' . $page, ...)`) is unsafe. If an array or unexpected structure is maliciously passed via the URL (e.g., `?page[]=1`), it leads to fatal type errors and cache key pollution.
**Action:** When using user input like query string parameters to construct cache keys in Laravel, strictly cast them to primitive types using helper methods like `$request->integer('page', 1)` or `$request->string('param')` rather than `$request->get()`. This enforces type safety and prevents application crashes.
