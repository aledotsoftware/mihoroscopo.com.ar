...
## 2026-06-09 - [Safe column filtering optimization on single record lookups]
**Learning:** When looking up a single Eloquent model record by email or external reference, using `first()` fetches all columns by default. If the table has heavy JSON columns like `response` or `payload`, hydrating these causes massive memory overhead. Passing explicit `$columns` to `select()` limits this overhead.
**Action:** When creating reusable getter methods for models (like `getSubscriptionByEmail`), allow passing an optional `$columns = ['*']` parameter. Callers that only need specific fields to check existence or update simple fields should pass an explicit list of columns to avoid heavy hydration overhead.
