...
## 2026-05-01 - [Avoid O(N) Switch Lookups in High Concurrency]
**Learning:** Large string-based `switch` statements perform O(N) evaluations, checking each case sequentially until a match is found. In controllers heavily invoked by automated traffic or webhooks, this can accumulate into unnecessary CPU cycle usage.
**Action:** Replace simple key-value `switch` mapping functions (e.g. `getCurrencyByCountry`) with a `static $map = [...]; return $map[$key] ?? $default;` pattern to ensure an O(1) constant-time lookup, optimizing performance and significantly improving readability.
