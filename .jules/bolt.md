...
## 2026-03-31 - [Avoid Optimizing Edit/Update Queries]
**Learning:** Do NOT restrict columns using `select()` or remove `JOIN`s when fetching a single record for an 'update' or 'edit' Blade view (e.g., in `SubscriptionController@update`). These views implicitly require the complete record (or large subsets) to populate form fields.
**Action:** Static analysis often misses which fields a Blade template uses. Omitting columns to save memory on edit views will cause critical UI regressions. Only use `select()` optimizations on Index/List views or headless API responses where the schema is strictly defined.
