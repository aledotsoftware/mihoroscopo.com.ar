...
## 2026-05-11 - [Optimize Eloquent Query Retrieval for Single Records]
**Learning:** Returning fully hydrated Eloquent models (via `->first()`) when only a subset of attributes (like `id` or `email`) is needed causes severe, localized memory overhead in high-throughput endpoints. This is especially problematic on tables containing large BLOB, TEXT, or JSON payload columns (e.g., the `response` and `payload` columns in `subscriptions`), leading to unnecessary memory allocation and potential out-of-memory errors during peak traffic.
**Action:** When creating reusable model-fetching methods (e.g., `getSubscriptionByEmail`), allow specifying a `$columns` array with a default `['*']`. This enables callers in memory-sensitive flows (like checkout or reactivation) to explicitly select only the required fields (`['id', 'email']`), bypassing the hydration of massive unused text/JSON columns.
