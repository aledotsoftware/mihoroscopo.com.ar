...
## 2026-06-09 - [Missing Index on Date column in Content Models]
**Learning:** The SendDailyContentEmails background job loops over thousands of records and filters 8 different content tables (like `content_astral_guide`, `content_love_prediction`, etc.) using the `date` column (e.g., `ContentAstralGuide::where('date', $date)->pluck(...)`). However, none of these tables had a database index on the `date` column. Over time, as historical content accumulates daily, this results in massive O(N) full table scans and significant CPU degradation on the database server during the job execution.
**Action:** Always ensure that columns used in high-frequency batch processing `WHERE` clauses (especially temporal fields like `date` or `created_at` in logging/content tables) are explicitly indexed in database migrations to guarantee O(log N) lookup performance.
