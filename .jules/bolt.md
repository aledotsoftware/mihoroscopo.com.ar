...
## 2026-04-22 - [Avoid Modifying Models Signatures Without Updating Callers]
**Learning:** When altering the signature of a helper method like `getSubscriptionByEmail` to take an optional `$columns` argument for performance optimizations, ensure all the internal DB queries chain the `select($columns)` securely and effectively pass exactly what is needed down the line. Failing to do so can result in unhydrated models (like excluding `status` or `external_reference`) breaking the logic paths immediately following the fetch.
**Action:** Always test the entire execution flow related to modified data extraction paths, to confirm the application functions consistently, and restrict scope changes specifically to the targeted files without using global formatters.
