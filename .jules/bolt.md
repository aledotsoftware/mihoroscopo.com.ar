...
## 2026-06-09 - [Selective Eloquent Column Hydration in Generic Getters]
**Learning:** Hardcoding `select()` limits inside generic controller getter methods (e.g., `getSubscriptionByEmail`) can silently break other callers that expect fully hydrated models. However, when these getters fetch records with massive JSON/TEXT columns during high-concurrency flows (like checkout or webhooks), fetching all columns wastes enormous RAM and CPU.
**Action:** Modify generic getter methods to accept an optional `$columns = ['*']` parameter. This maintains backward compatibility for existing callers while allowing high-throughput endpoints to safely restrict queries via `select()` (e.g., `$this->getSubscriptionByEmail($email, ['id', 'status', 'external_reference'])`). Since Eloquent's `save()` only updates dirty attributes, omitting columns in the select statement is perfectly safe and won't overwrite unselected data with null.
