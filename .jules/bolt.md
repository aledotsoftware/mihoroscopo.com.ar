...
## 2024-06-20 - Synchronous HTTP in Notification Workflows
**Learning:** External API requests like Discord notifications via Guzzle, when executed synchronously within webhook controllers (e.g. `NotificationController`) or command jobs, block the main HTTP thread. In a high-throughput endpoint like `toQueue`, this leads to increased latency and potential timeouts from webhook providers like MercadoPago. `DiscordService` is called frequently within these webhooks, compounding the impact. While `NotificationController::toQueue` already defers the main webhook processing block, deferring at the service level (`DiscordService::sendDiscordMessage`) guarantees that any other caller utilizing this service also receives the non-blocking background execution benefit without needing to remember to defer the call manually, adhering to single-responsibility and performance patterns in this codebase.
**Action:** Always wrap non-critical external synchronous API calls (logging, analytics, chat notifications) inside Laravel 11's `defer()` helper within the service method itself. This guarantees all consumers automatically get the performance benefit and isolates background execution concerns. Ensure error logging switches from `echo` to `\Log::error()` since deferred execution output will not be visible on the web response stream.
