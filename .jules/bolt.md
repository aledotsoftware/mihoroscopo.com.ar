...
## 2024-05-19 - Replacing Model::create() with DB::table()->insert() in loops and endpoints
**Learning:** The `email_logs` table (and its `EmailLog` model via `public $timestamps = false;`) doesn't use standard Eloquent `created_at` or `updated_at` fields, relying instead on `sent_at` and `opened_at`. Adding standard timestamps manually when migrating to `DB::table()->insert()` causes "Unknown column" SQL errors. Conversely, the `email_clicks` table does require timestamps, meaning `DB::table('email_clicks')->insert()` must manually specify `created_at` and `updated_at` to avoid "doesn't have a default value" exceptions.
**Action:** When replacing `Model::create()` with raw `DB::table()->insert()` for performance, rigorously verify the model's timestamp configuration (`public $timestamps`) and the actual database schema before blindly adding or omitting `created_at` and `updated_at` properties in the insert array.
