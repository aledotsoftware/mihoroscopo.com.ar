...
## 2026-05-18 - [Eloquent Model Hydration Overhead on High Concurrency Flow]
**Learning:** High-concurrency controller endpoints, like webhooks or subscription flows (`SubscriptionController`), that fetch Eloquent models (e.g., via `first()`) from tables containing massive TEXT or JSON columns (such as `response` and `payload` in the `subscriptions` table) will suffer significant CPU spikes and memory consumption issues if these large columns are unnecessarily hydrated.
**Action:** When querying models in high-concurrency read/write paths, explicitly specify required columns using an optional `$columns` parameter passed to Eloquent's `select()` method (e.g., in generic getter methods like `getSubscriptionByEmail`), bypassing the hydration of expensive, unneeded columns.
