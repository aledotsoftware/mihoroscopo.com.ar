...
## 2026-03-30 - [Caching Static Eloquent Models]
**Learning:** In endpoints that serve static content directly from the database based on a slug (e.g., `PageController::show`), performing `Page::where('slug', $slug)->firstOrFail()` repeatedly for every request wastes database resources and adds unnecessary latency, especially if the tables contain text content.
**Action:** When a controller fetches static resources that change very rarely, wrap the Eloquent model retrieval inside `Cache::remember()` using a cache key based on the unique identifier (like the slug). This eliminates redundant queries and improves response time.
