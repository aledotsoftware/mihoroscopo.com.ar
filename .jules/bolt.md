...
## 2026-05-15 - Optimizing Generic Getters in High-Concurrency Flows
**Learning:** Hardcoding `select()` into a generic getter method like `getSubscriptionByEmail` silently breaks other callers that might expect fully-hydrated model properties. However, during high-concurrency flows (like checkout/webhooks), fetching massive TEXT/JSON columns ('response', 'payload') from tables like `subscriptions` into memory for full hydration causes severe memory overhead and stalls performance.
**Action:** When optimizing models with massive columns, modify generic getter method signatures to accept an optional `$columns = ['*']` parameter. This allows callers in performance-critical flows to explicitly request only the subset of columns they need for logic checks, bypassing large payload columns safely without breaking default functionality.
