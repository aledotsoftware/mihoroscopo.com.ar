...
## 2026-04-28 - [Safe select() optimization for single record updates]
**Learning:** When optimizing Eloquent first() lookups with select() to avoid hydrating massive JSON columns, hardcoding select() into generic getter methods (e.g., getSubscriptionByEmail) is an anti-pattern as returning partially hydrated models silently breaks other callers.
**Action:** Instead, modify the getter signature to accept an optional $columns = ['*'] parameter so specific high-concurrency flows can restrict columns safely.
