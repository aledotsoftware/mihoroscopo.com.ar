...
## 2026-06-26 - Replaced O(N) switch with O(1) array map
**Learning:** In high-throughput endpoints or batch processing contexts in PHP, replacing large string-based `switch` statements mapping simple key-value pairs with a `static` array lookup significantly improves code readability and provides an O(1) constant-time lookup, saving CPU cycles.
**Action:** When identifying large key-value mapping logic using `switch`, refactor to use `static $map = [...]; return $map[$key] ?? $default;` to improve execution time and maintainability.
