...
## 2026-05-20 - Consolidating Actions and Making Table Admin Buttons Accessible
**Learning:** Found redundant plain text actions next to non-functional mock icon buttons in the admin articles table. The mock buttons also lacked proper accessibility labels.
**Action:** Merged the functionality into the beautiful icon buttons. Added `aria-hidden="true"` to the inner SVG icons and added specific, context-aware `aria-label`s to the buttons themselves (e.g., `aria-label="Editar artículo: {{ $article->title }}"`). Also properly escaped variables passed to inline JS confirm dialogs with `addslashes()`.
