...
## 2024-05-28 - Missing visual required indicators and dynamic titles in confirm dialogs
**Learning:** Admin forms lacked visual `*` indicators despite having native `required` attributes, which degraded UX for sighted users. The `confirm()` deletion dialogs in data tables used generic text (e.g. `¿Estás seguro de que deseas eliminar este artículo?`) lacking context of what was being deleted.
**Action:** Add `<span class="required-indicator" aria-hidden="true">*</span>` explicitly tied to the field's `<label>` when the field is marked as required. Also, safely inject dynamic item titles into inline JavaScript `confirm()` dialogs using `data-*` attributes (e.g., `data-title="{{ $model->title }}"` and `this.dataset.title`) to prevent HTML breakage or XSS vulnerabilities.
