...
## 2026-04-24 - Add alt text to layout images
**Learning:** Image elements in repeating UI elements like dropdowns (e.g., `dropdown-user-img`, `dropdown-notifications-item-img`) were missing `alt` tags, which is an accessibility violation that causes screen readers to read the file path instead.
**Action:** Always verify that `<img>` tags include an `alt` attribute. Use descriptive text (e.g., `alt="User avatar"`) or empty string `alt=""` if purely decorative.
