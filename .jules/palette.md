...
## 2024-11-25 - Functional Template Buttons
**Learning:** Admin templates often provide beautiful mock buttons (e.g. icon-only action links) that developers sometimes leave broken while adding redundant, plain-text functional links next to them.
**Action:** Always integrate the functional backend logic (routes, form methods) into the styled template elements, and enhance them with descriptive `aria-label`s containing the item context (like title) and confirmation dialogs for destructive actions.
