...
## 2024-11-26 - Merging Template Mock Elements with Backend Functionality
**Learning:** When adopting HTML admin templates, visually appealing UI elements (like SVG-based icon buttons) are often hardcoded with dummy links (e.g., `href="#!"` or `href="edit-post.html"`). Developers sometimes leave these broken buttons alongside plain-text, functional backend links (e.g., `<a href="/admin/edit">Editar</a>`). This creates visual clutter, redundant tab stops for screen readers, and broken functionality.
**Action:** Always fully integrate template UI mockups by explicitly migrating actual backend URLs, dynamic state (like IDs), and route methods (e.g., `<form action="/.../id">`) into the styled template elements. Delete any leftover plain-text functional placeholders.
